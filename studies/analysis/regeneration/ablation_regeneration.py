...
from gibbsq.qroute.utils.chart_exporter import save_data
from gibbsq.qroute.utils.run_artifacts import artifacts_dir, config_path, figure_path, metrics_path

from gibbsq.experiments.evaluation.n_gibbsq_evals.ablation_ssa import (
    ALL_VARIANTS,
    NEURAL_VARIANTS,
    REFERENCE_VARIANTS,
...
class AblationRecord:
    ...
    delta_vs_reflected_uas_mean: float | None = None
    delta_vs_reflected_uas_se: float | None = None
    delta_vs_reflected_uas_ci95_half_width: float | None = None
    ...
    for payload in ordered_rows:
        ...
        validated.append(
            AblationRecord(
                ...
                delta_vs_reflected_uas_mean=_optional_float(payload, "delta_vs_reflected_uas_mean"),
                delta_vs_reflected_uas_se=_optional_float(payload, "delta_vs_reflected_uas_se"),
                delta_vs_reflected_uas_ci95_half_width=_optional_float(payload, "delta_vs_reflected_uas_ci95_half_width"),
...
def _load_experiment_config(run_dir: Path) -> ExperimentConfig:
    ...
    from gibbsq.qroute.core.config import validate
...
def build_ablation_plot_payload(records: Iterable[AblationRecord]) -> dict:
    ...
    return {
        ...
        "delta_vs_reflected_uas_mean": [record.delta_vs_reflected_uas_mean for record in ordered],
