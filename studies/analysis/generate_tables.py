...
Usage:
    python -m studies.analysis.generate_tables
    python -m studies.analysis.generate_tables --data-dir outputs/data
    python -m studies.analysis.generate_tables --table-dir outputs/tables
"""
...
def table_boundary_equilibrium(data_dir: Path, table_dir: Path) -> list[Path]:
    ...
    if csv_path is None:
        log.warning("No boundary_equilibrium_verification CSV — skipping")
        return []
...
def table_reflected_ode(data_dir: Path, table_dir: Path) -> list[Path]:
    ...
    if csv_path is None:
        log.warning("No reflected_ode_convergence_summary CSV — skipping")
        return []
...
def table_drift_audit(data_dir: Path, table_dir: Path) -> list[Path]:
    ...
    if csv_path is None:
        log.warning("No exhaustive_drift_summary CSV — skipping")
        return []
...
def table_boundary_mismatch(data_dir: Path, table_dir: Path) -> list[Path]:
    ...
    if csv_path is None:
        log.warning("No ctmc_boundary_mismatch_summary CSV — skipping")
        return []
