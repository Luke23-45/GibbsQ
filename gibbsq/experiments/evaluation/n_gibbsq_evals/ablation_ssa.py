...
NEURAL_VARIANTS: list[AblationVariantSpec] = [
    ...
    AblationVariantSpec(
        name="BC from Reflected UAS -> REINFORCE",
        variant_kind="neural",
        panel="teacher",
        preprocessing="log1p",
        init_type="standard",
        bootstrap_mode="expert",
        expert_policy_name="reflected_uas",
        expert_policy_params={"alpha": 20.0, "beta": 0.85, "gamma": 0.5, "c": 0.5},
        artifact_dir="variant_4_bc_from_reflected_uas_to_reinforce",
    ),
    AblationVariantSpec(
...
def _summary_row(
    ...
    metrics: dict,
    *,
    delta_vs_reflected_uas: dict[str, float] | None,
    ...
    if delta_vs_reflected_uas is not None:
        row.update(
            {
                "delta_vs_reflected_uas_mean": delta_vs_reflected_uas["mean"],
                "delta_vs_reflected_uas_se": delta_vs_reflected_uas["se"],
                "delta_vs_reflected_uas_ci95_half_width": delta_vs_reflected_uas["ci95_half_width"],
            }
        )
...
def run_ablation(
    ...
    reflected_metrics = summary_metrics["Reflected UAS"]
    ...
    for spec in ALL_VARIANTS:
        ...
        delta_vs_reflected = _compute_delta_metrics(metrics["q_totals"], reflected_metrics["q_totals"])
        delta_vs_best_neural = _compute_delta_metrics(metrics["q_totals"], best_neural_metrics["q_totals"])
        row = _summary_row(
            spec,
            metrics,
            delta_vs_reflected_uas=delta_vs_reflected,
            delta_vs_best_neural=delta_vs_best_neural,
        )
        summary_rows.append(row)
        append_metrics_jsonl(row, metrics_jsonl_path)
        log.info(
            "  %-30s E[Q_total] = %.4f +/- %.4f | delta vs Reflected UAS = %+0.4f",
            spec.name,
            row["mean_q_total"],
            row["se_q_total"],
            row["delta_vs_reflected_uas_mean"],
        )
