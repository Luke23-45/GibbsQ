...
from gibbsq.qroute.engines.jax_engine import compute_configured_max_events, policy_name_to_type, run_replications_jax
...
def _policy_label(name: str) -> str:
    mapping = {
        ...
        "jssq": "JSSQ",
        "uas": "UAS",
        "reflected_uas": "Reflected UAS",
        "refined_uas": "Reflected UAS",
    }
...
def _policy_contract_alpha(policy_name: str, default_alpha: float) -> float:
    if policy_name == "uas":
        return 10.0
    if policy_name in {"reflected_uas", "refined_uas"}:
        return 20.0
...
def _build_scenarios(raw_cfg: DictConfig, profile_name: str, cfg: ExperimentConfig) -> list[ScenarioSpec]:
    ...
    if ep.include_stats_contract:
        ...
        scenarios.append(
            ScenarioSpec(
                contract="stats",
                name="stats/reflected_uas",
                policy_name="reflected_uas",
                label="stats::Reflected UAS",
        ...
        for idx, (scale, rho) in enumerate(cells):
            ...
            scenarios.append(
                ScenarioSpec(
                    ...
                    policy_name="reflected_uas",
                    label=f"generalize::Reflected UAS::{scale}x::{rho:.2f}",
        ...
        for idx, rho in enumerate(rhos):
            scenarios.append(
                ScenarioSpec(
                    ...
                    policy_name="reflected_uas",
                    label=f"critical::Reflected UAS::{rho:.3f}",
