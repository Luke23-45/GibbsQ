#!/usr/bin/env python3
"""
Exploratory proof-search workflow for the full Reflected UAS family.
...
from gibbsq.qroute.core.config import ExperimentConfig, _profile_path, hydra_to_config, load_experiment_config, validate  # noqa: E402
from gibbsq.qroute.core.policies import ReflectedUASRouting  # noqa: E402
...
DEFAULT_OUTPUT_DIR = "outputs/reflected_uas_proof_search"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Search for numerically promising proof templates for the full Reflected UAS family.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--config-name", default=DEFAULT_CONFIG_NAME)
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--alpha", type=float, default=20.0, help="Reflected-policy alpha used in the search.")
    parser.add_argument(
        "--policy-grid",
        choices=["audit", "coarse64"],
        default="coarse64",
        help="Which set of reflected parameter points to scan.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    log.info("Reflected UAS proof-search capsule: %s", run_dir)
