"""
REINFORCE policy-gradient training for the active z2 neural-support path.

This module implements the current SSA-based neural training pipeline used by
the H7 applicability studies.
...
from gibbsq.qroute.core.policy_distribution import compute_numpy_policy_probs
from gibbsq.qroute.core.reinforce_objective import compute_action_interval_returns_from_trajectory_numpy
from gibbsq.qroute.core.features import look_ahead_potential
from gibbsq.qroute.core.policies import JSQRouting, JSSQRouting
...
from gibbsq.qroute.utils.model_io import (
    BC_POINTER,
...
class TrajectoryResult(NamedTuple):
    ...
    log_probs: list[float]  # log Ï€_Î¸(a_t | s_t) for each routing decision
...
def compute_gae(
    ...
    - Î»=0: Low variance, high bias (TD(0))
    - Î»=1: High variance, low bias (Monte Carlo returns)
    - Î»=0.95: Optimal tradeoff (Schulman recommendation)

    For CTMC, we compute TD residuals using continuous-time discounting:
        Î´_t = r_t + Î³^(Î”t) * V(s_{t+1}) - V(s_t)
    ...
    # 5. Compute action-level GAE: A_k = Î£_l (Î³Î»)^{Î”T_{k:k+l}} Î´_{k+l}
...
def collect_trajectory_ssa(
    ...
    simulation runs as the true CTMCâ€”no smoothing, no relaxation.
    ...
    arrival_rate : float
        Arrival rate Î».
    service_rates : np.ndarray
        Service rates Î¼_i for each server.
    sim_time : float
    ...
    while t < sim_time:
        ...
        if a0 <= constants.RATE_GUARD_EPSILON:
            break  # Degenerateâ€”no events possible
...
class ReinforceTrainer:
    ...
    def bootstrap_from_expert(self, policy_net, value_net, key, jsq_limit, random_limit, denom):
        ...
        from gibbsq.qroute.core.pretraining import train_robust_bc_policy, train_robust_bc_value
    ...
    def _save_assets(
        ...
        log.info(f"Deterministic Policy Score: {final_mean:.2f}% Â± {final_std:.2f}%")
