...
policy-comparison metrics for JSSQ / UAS / Reflected UAS, evaluates only new
...
from gibbsq.qroute.core.policies import StateDependentUASRouting
...
def _write_summary(
    ...
    *,
    ...
    lines = [
        ...
        f"- Reflected UAS reference: `{references.get('Reflected UAS', float('nan')):.6f}`" if references else "- Reflected UAS reference: unavailable",
        "",
        "## Finalists",
        "",
    ]
...
def main(raw_cfg: DictConfig) -> None:
    ...
    if best is not None:
        ...
        cal_uas = references.get("Reflected UAS")
        ...
        if drift_ok and cal_uas is not None and best_q < cal_uas:
            recommendation = (
                ...
                "Reflected UAS benchmark while preserving the sampled theorem-style drift bound."
            )
        elif drift_ok and jssq is not None and best_q < jssq:
            recommendation = (
                "Proceed cautiously: the best adaptive-temperature UAS candidate beat "
                "JSSQ but did not clear the saved Reflected UAS benchmark."
            )
        elif not drift_ok:
            recommendation = (
