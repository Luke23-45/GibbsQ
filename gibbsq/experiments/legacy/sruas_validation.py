#!/usr/bin/env python3
"""
SRUAS validation runner for theorem certification and reflected-policy reruns.

This script provides a dedicated workflow for the manuscript update that
introduces a theorem-backed Stability-Certified Reflected UAS (SRUAS)
subfamily without overclaiming the full empirical Reflected UAS family.

Modes
-----
`audit`
    Compute the SRUAS theorem constants on the active benchmark setup.
`rerun`
    Rerun the anchor policy comparison for UAS, empirical Reflected UAS,
    and only those SRUAS candidates with epsilon > 0.
`full`
...
def default_candidate_catalog() -> list[CandidateSpec]:
    return [
        CandidateSpec(
            ...
            gamma=0.5,
            c=0.5,
            family="empirical_reflected_default",
            notes="Current manuscript default empirical reflected policy.",
        ),
        CandidateSpec(
            name="sruas_candidate_b0p5_g0p25_c0p25",
            beta=0.5,
            gamma=0.25,
            c=0.25,
            family="sruas_candidate",
            notes="Grid candidate proposed for theorem audit.",
        ),
        CandidateSpec(
            name="sruas_candidate_b0p5_g0p5_c0p25",
            beta=0.5,
            gamma=0.5,
            c=0.25,
            family="sruas_candidate",
        ...
        CandidateSpec(
            name="sruas_candidate_b0p7_g0p25_c0p25",
            beta=0.7,
            gamma=0.25,
            c=0.25,
            family="sruas_candidate",
    ...
    for raw_triplet in extra_triplets:
        ...
        catalog.append(
            CandidateSpec(
                ...
                c=c,
                family="sruas_candidate",
                notes="User-specified SRUAS candidate.",
            )
        )
        known.add(triplet)
    return catalog
...
def _validate_protocol(cfg: ExperimentConfig, protocol: ValidationProtocol) -> None:
    ...
    if protocol.alpha_uas <= 0 or protocol.alpha_reflected <= 0:
        raise ValueError("alpha_uas and alpha_reflected must both be > 0.")
...
def certified_sruas_candidates(
    audit_rows: Sequence[dict[str, object]],
) -> list[dict[str, object]]:
    return [
        row
        for row in audit_rows
        if row["family"] == "sruas_candidate" and bool(row["is_certified"])
    ]
...
def evaluate_policy(
    *,
    ...
    results = run_replications(
        ...
        progress_desc=f"sruas policy eval ({label})",
    )
...
def build_policy_suite(
    *,
    ...
    for row in certified_sruas_candidates(audit_rows):
        label = (
            f"SRUAS (beta={row['beta']}, gamma={row['gamma']}, c={row['c']})"
        )
        suite.append(
            (
                label,
...
def render_piecewise_summary_markdown(
    *,
    ...
    lines = [
        ...
        "",
        "This report documents a numerically verified piecewise Lyapunov candidate for the benchmark-default reflected policy.",
...
def run_piecewise_candidate_verification(
    *,
    ...
    for round_idx in range(piecewise_protocol.max_refinement_rounds):
        ...
        if combo is None:
            raise RuntimeError("Failed to construct a convex tail candidate for the default reflected policy.")
...
def run_audit(
    *,
    ...
    audit_rows = audit_candidates(
        ...
        alpha_reflected=protocol.alpha_reflected,
    )
    audit_log_path = metrics_path(run_dir, "sruas_audit.jsonl")
    for row in audit_rows:
    ...
    metadata_path(run_dir, "sruas_audit_summary.json").write_text(
...
def run_rerun(
    *,
    ...
    rerun_log_path = metrics_path(run_dir, "sruas_policy_comparison.jsonl")

    if not certified_sruas_candidates(audit_rows):
        log.warning(
            "No SRUAS candidates satisfied epsilon > 0 under the active benchmark. "
            "Rerun will include only UAS and the empirical reflected default."
        )
    ...
    metadata_path(run_dir, "sruas_policy_summary.json").write_text(
        json.dumps(
            {
                "system": compute_system_summary(cfg),
                "protocol": asdict(protocol),
...
def write_summary_files(
    *,
    ...
    metadata_path(run_dir, "sruas_validation_summary.md").write_text(
        summary_md,
        encoding="utf-8",
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the mandatory SRUAS theorem audit and the reflected-only rerun.",
    ...
    parser.add_argument(
        "--output-dir",
        default="outputs/post_sruas_validation",
    ...
    parser.add_argument(
        "--alpha-reflected",
        type=float,
        default=DEFAULT_REFLECTED_ALPHA,
        help="Alpha used for empirical Reflected UAS and SRUAS candidates.",
    )
    parser.add_argument(
        "--candidate",
        action="append",
        default=[],
        help="Additional SRUAS candidate triplet formatted as beta,gamma,c.",
    )
    parser.add_argument(
        "--piecewise-tail-threshold",
        type=float,
        default=20.0,
        help="Tail threshold used when searching the default reflected piecewise candidate.",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir, run_id = get_run_config(cfg, "sruas_validation", resolved_raw)
    log.info("SRUAS validation capsule: %s", run_dir)
    ...
    if args.mode in {"audit", "full", "rerun", "all"}:
        ...
        if not certified_sruas_candidates(audit_rows):
            log.warning(
                "Audit found no theorem-certified SRUAS candidates for the active setup."
            )
