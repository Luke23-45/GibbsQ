...
where the advantage surrogate A_i(Q) is the reflected-UAS energy plus
non-negative convex value corrections. The family contains Reflected UAS
exactly, so this probe answers the only decision question that matters:

    does any value-routing candidate beat Reflected UAS on the anchor
...
from gibbsq.qroute.core.policies import (
    ReflectedUASRouting,
    JSSQRouting,
...
def _write_summary(
    ...
    *,
    ...
    lines = [
        ...
        f"- Saved Reflected UAS: `{references.get('Reflected UAS', {}).get('mean_q_total', float('nan')):.6f}`" if references else "- Saved Reflected UAS: unavailable",
        "",
        "## Audit",
        "",
    ...
    for row in finalists:
        ...
        if delta is not None:
            lines.append(
                f"delta vs Reflected UAS for `{row['label']}`: `{delta['mean_delta']:+.6f}` "
...
def main(raw_cfg: DictConfig) -> None:
    ...
    baseline_specs = [
        ...
        ("Reflected UAS", ReflectedUASRouting(mu, alpha=20.0, beta=0.85, gamma=0.5, c=0.5)),
    ]
    ...
    paired_deltas: dict[str, dict[str, float]] = {}
    reflected_q = np.asarray(baseline_metrics["Reflected UAS"]["q_totals"], dtype=np.float64)
    ...
    for finalist in finalists:
        ...
        paired_deltas[str(finalist["label"])] = {
            "vs_reflected": _paired_delta(q_totals, reflected_q),
    ...
    if best_delta["ci95_high"] < 0.0 and has_value_correction:
        ...
        recommendation = (
            ...
            "just a re-tuned reflected baseline rather than evidence for the new "
            "value-routing architecture."
        )

    paired_summary = {
        label: {
            "vs_reflected": delta["vs_reflected"],
    ...
    _write_summary(
        ...
        baselines=baseline_rows,
        finalists=final_rows,
        paired_deltas={label: delta["vs_reflected"] for label, delta in paired_deltas.items()},
        recommendation=recommendation,
    )
