...
Direction 1:
    Numerical fluid-limit diagnostics for the existing Reflected UAS policy.
    This does not search for a better policy; it tests whether the reflected
...
from gibbsq.qroute.core.policies import RoutingPolicy
...
class SparsemaxEnergyRouting:
    """Fenchel-Young proxy policy using sparsemax over reflected-UAS logits."""
...
def _projected_fluid_step(
    ...
    *,
    ...
    probs = _reflected_uas_prob(q, mu, alpha=alpha, beta=beta, gamma=gamma, c=c)
    arrivals = lam * probs
...
def _write_summary(
    ...
    *,
    ...
    cal = references.get("Reflected UAS", {})
    ...
    lines = [
        ...
        f"- Reflected UAS mean_q_total: `{cal.get('mean_q_total', float('nan')):.6f}`" if cal else "- Reflected UAS mean_q_total: unavailable",
        f"- JSSQ mean_q_total: `{jssq.get('mean_q_total', float('nan')):.6f}`" if jssq else "- JSSQ mean_q_total: unavailable",
        "",
        "## Direction 1",
        "",
        f"- Policy performance proxy: existing Reflected UAS value `{cal.get('mean_q_total', float('nan')):.6f}`",
...
def main(raw_cfg: DictConfig) -> None:
    ...
    cal_ref = references.get("Reflected UAS")
    ...
    if direction2_enabled and final_rows and cal_ref:
        ...
        if d2_mean + material_margin < cal_mean:
            recommendation = (
                ...
                "the saved Reflected UAS benchmark by more than the combined noise margin."
            )
        elif bool(direction1["supports_single_attractor"]):
            recommendation = (
                "Choose Direction 1: Direction 2 did not beat Reflected UAS, while "
                "the fluid diagnostic supports pursuing certification of the existing empirical winner."
            )
        else:
            recommendation = (
                "Do not commit yet: Direction 2 failed to beat Reflected UAS and "
