...
from gibbsq.qroute.core.pretraining import DEFAULT_BC_DATA_CONFIG, extract_bc_data_config
from gibbsq.qroute.utils.logging import get_run_config
...
def summarize_policy_results(results: dict[str, Any]) -> dict[str, Any]:
    ...
    reflected = results.get("Reflected UAS", {})
    return {
        "parity": parity,
        ...
        "reflected_uas_mean_q_total": float(reflected.get("mean_q_total", float("inf"))),
    }
...
def _run_policy_trial(raw_candidate: DictConfig, profile_name: str) -> dict[str, Any]:
    from gibbsq.experiments.evaluation.baselines_comparison import run_corrected_comparison
...
def _run_generalization_trial(raw_candidate: DictConfig, profile_name: str) -> dict[str, Any]:
    from gibbsq.experiments.evaluation.n_gibbsq_evals.gen_sweep import GeneralizationSweeper
...
def _run_critical_trial(raw_candidate: DictConfig, profile_name: str) -> dict[str, Any]:
    from gibbsq.experiments.evaluation.n_gibbsq_evals.critical_load import CriticalLoadTest
...
def _run_stress_trial(raw_candidate: DictConfig, profile_name: str) -> dict[str, Any]:
    from gibbsq.experiments.testing import stress_test as stress_module
