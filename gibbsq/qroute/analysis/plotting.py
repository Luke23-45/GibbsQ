...
from gibbsq.qroute.core.drift import DriftResult
from gibbsq.qroute.analysis.metrics import running_average, total_queue_trajectory
from gibbsq.qroute.engines.numpy_engine import SimResult
from gibbsq.qroute.analysis.theme import apply_theme, get_current_theme, THEMES
from gibbsq.qroute.analysis.chart_styles import (
    ChartType,
    ChartStyleSpec,
    get_chart_style,
    get_semantic_colors,
...
from gibbsq.qroute.analysis.plot_profiles import (
    ExperimentPlotContext,
    ExperimentPlotProfile,
    resolve_experiment_plot_profile,
)
from gibbsq.qroute.utils.chart_exporter import save_chart, ChartConfig
...
def plot_improvement_heatmap(
    ...
    *,
    ...
    cbar = plt.colorbar(
        im,
        ax=ax,
        label=plot_profile.axis_labels.get("colorbar", "Improvement Ratio (Reflected UAS / Neural)"),
    )

    cbar.ax.set_title("Reflected UAS\nSuperior", fontsize=spec.annotation_fontsize - 1, fontweight="bold", pad=12)
