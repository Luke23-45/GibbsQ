...
def collect_robust_expert_data(
    ...
    for rho in rhos:
        ...
        for s in states:
            ...
            # PATCH: Use state-dependent queue sum, not constant mean_q
            # This aligns pretraining targets with RL critic targets
            ...
            # PATCH: Use state-dependent queue sum for augmented states too
...
def train_robust_bc_value(
    ...
    # PATCH: Apply tanh scaling to targets to match RL critic targets
    # G contains state-dependent queue sums, scale to same range as RL
    G_scaled = 100.0 * jnp.tanh(G / 500.0)
    ...
    for i in range(num_steps + 1):
        value_net, opt_state, loss = step(value_net, opt_state, X, R, G_scaled)
        if i % 100 == 0:
