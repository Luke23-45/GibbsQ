...
## 10. Calibrated Empirical Extension

The codebase also includes a stronger closed-form benchmark candidate:
...
With \((\beta,\gamma,c)=(1,1,1)\), this reduces exactly to UAS. The current benchmark-tuned default used for isolated policy comparisons is
...
- it shifts the look-ahead offset from \(1\) to \(c\)
