...
Thus, the project can be completed with a rigorous stochastic stability proof by introducing a scaling argument. This would make the paper state-of-the-art: it proves that Reflected UAS (with appropriately scaled temperature, which can be set to the empirical best) is stable.
...
The theorem then asserts: **Under the benchmark parameters, Reflected UAS with \(\alpha=20\) is positive Harris recurrent.**
