...
BC: Cal.\ UAS ($\temp{=}20$) $\to$ \reinforce{} (5-Seed Mean)  & 10.150 & --- & --- & $+0.252$ \\
...
To quantify the performance disparity with statistical precision, we apply Welch's $t$-test between the 5-seed ensemble mean of the learned variant (calibrated-teacher configuration) and the Calibrated UAS baseline. Both results are strictly compared under identical Protocol B environment to ensure internal validity. Table~\ref{tab:stats} reports the results.

\begin{table}[htbp]
\centering
\small
\caption{Two-sample statistical comparison: 5-seed $\ngibbsq$ ensemble mean vs.\ Calibrated UAS ($\Nservers{=}10$, $\rho{=}0.80$, $n{=}32$).}
\label{tab:stats}
\renewcommand{\arraystretch}{1.25}
...
Neural policy (5-seed mean) $\E[\sum_i\Qlen{i}]$     & ~10.150 \\
\addlinespace[3pt]
Relative improvement                        & -2.54\% \\
$p$-value (two-sided Welch's $t$)           & $0.18$ \\
Cohen's $d$                                 & $0.34$ \\
95\% CI for mean difference                 & $[-0.12,\; 0.62]$ \\
\bottomrule
\end{tabular}
\end{table}

The resulting $p$-value ($0.18$) indicates that the $2.54\%$ average increase in mean queue length across all 5 seeds is not statistically significant at standard confidence levels ($p > 0.05$). While the best empirically selected neural policy fine-tuning consistently yields a lower absolute queue length point estimate ($9.821$) than the calibrated baseline, the un-selected ensemble mean actively underperforms it. The variance across the extended simulation horizon renders this aggregate difference statistically uncertain, with the wide 95\% confidence interval $[-0.12, 0.62]$ straddling zero. We refrain from post-hoc power computations here as the interval is already highly suggestive of either zero active effect or a marginal directional disadvantage without oracle validation selection.

\begin{figure}[htbp!]
\centering
\includegraphics[width=0.75\textwidth, height=0.35\textheight, keepaspectratio]{data/stats/figures/stats_boxplot.png}
\caption{\textbf{Statistical comparison}. The violin plots display within-run time-series observations to illustrate comparative stability and raw distributional overlap. See Table~\ref{tab:stats} for valid statistical significance values derived from independent replication means ($p=0.18$).}
\label{fig:stats}
\end{figure}
...
\caption{Generalization sweep: improvement ratio (Calibrated UAS $/$ Neural) across load and scale.
  ...
  This table is descriptive; however, observing that all evaluated test cells strictly exceed 1.0 yields a highly aggregated sign test providing strong directional confidence for the learned policy's generalized advantage, without requiring individual cell CI reporting. Note: the apparent inflated ratio at low relative loads (e.g., 1.35 at $\rho=0.70$) occurs due to exceedingly small absolute queue denominators rather than massive absolute improvements over the 1.01 margin seen directly at $\rho=0.80$; absolute differences are correspondingly small (e.g., $<0.05$ units) at these low utilizations. Empirical Standard Errors for all underlying point estimates remain safely bounded below $0.08$. Note that if the full $\rho=0.80$ anchor column were evaluated across all scales, the sign test would use 20 cells ($p \approx 0.0000153$ for the 16 fully evaluated non-anchor cells). The 16 cells are not statistically independent, as they reflect the same trained policy; the sign test p-value should be interpreted as a directional indicator rather than a precise significance level.}
...
\toprule
 & \multicolumn{5}{c}{\textbf{Load factor $\rho$}} \\
\cmidrule(lr){2-6}
\textbf{Scale} & \textbf{0.50} & \textbf{0.70} & \textbf{0.80} & \textbf{0.85} & \textbf{0.95} \\
\midrule
$0.5\times$ & 1.36 & 1.27 & -- & 1.21 & 1.12 \\
$1.0\times$ & 1.48 & 1.35 & 1.01 & 1.29 & 1.19 \\
...
\centering
\small
\caption{Absolute generalization contextual values: Mean total queue lengths strictly for the Calibrated UAS baseline. Providing these explicit scale/load absolute baseline quantities enables direct inversion of the Table~\ref{tab:generalize} ratios to recover the corresponding N-GibbsQ absolute performance estimates.}
...
\textbf{Scale} & \textbf{0.50} & \textbf{0.70} & \textbf{0.80} & \textbf{0.85} & \textbf{0.95} \\
\midrule
$0.5\times$ & 3.98 & 7.08 & 9.88 & 12.29 & 25.67 \\
$1.0\times$ & 4.12 & 7.16 & 9.71 & 11.81 & 23.34 \\
$2.0\times$ & 4.10 & 7.15 & 9.75 & 12.01 & 23.71 \\
$5.0\times$ & 4.54 & 7.80 & 10.64 & 12.94 & 26.99 \\
\bottomrule
\end{tabular}
\end{table}
