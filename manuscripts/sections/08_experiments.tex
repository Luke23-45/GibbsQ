...
95\% paired-t CI for $\Delta$\footnotemark  & $[-0.102,\; -0.051]$ \\
\bottomrule
\end{tabular}
\end{table}
\footnotetext{The paired-t test assumes independent replications; the best-of-5 seed selection may violate this assumption, inflating the apparent significance.}
...
\centering
\small
\caption{\textbf{Near-critical load:} Mean $\E[\sum_i\Qlen{i}]$ and relative improvement ($\Nservers{=}10$, 2 replications per $\rho$). \textbf{WARNING:} The reported Standard Errors (SE) in this table are explicitly theoretical lower bounds derived from conservative $(1-\rho)^{-2}$ asymptotic scaling properties. True empirical confidence intervals under these extreme near-boundary mixing times are significantly larger; the reported improvement percentages should be interpreted as upper bounds on the true improvement.}
\label{tab:critical}
\renewcommand{\arraystretch}{1.25}
