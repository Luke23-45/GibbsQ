...
Post-validation theorem auditing, however, shows that the benchmark-default calibrated point $(0.85, 0.5, 0.5)$ has $\varepsilon_{\calbeta,\calgamma,\caloffset} = -1.199$ under this protocol, and the three audited candidate points are likewise uncertified; see Table~\ref{tab:scuas_audit}.
...
\item \textbf{Calibrated-teacher variant achieves the lowest queue length:}
  At $9.82$, it surpasses both the Calibrated UAS reference ($9.90$) and JSSQ ($10.83$) under this protocol. Notably, the strongest neural result is not the default learned policy configuration.
\end{enumerate}
