...
Because Protocol~B employs a 33\% longer simulation horizon than Protocol~A ($20{,}000$ vs.\ $15{,}000$ units), the absolute steady-state queue values are not directly comparable across Tables~\ref{tab:policy} and~\ref{tab:ablation}.
Reference policies are re-evaluated under Protocol~B for internal comparability.
...
The resulting $p$-value ($0.39$) indicates that the $0.78\%$ average reduction in mean queue length is not statistically significant at standard confidence levels ($p > 0.05$). While the neural policy gradient fine-tuning consistently yields a lower absolute queue length point estimate than the calibrated baseline, the variance across the extended simulation horizon renders this aggregated difference statistically uncertain. 
