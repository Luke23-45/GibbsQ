...
95\% paired-t CI for $\Delta$                 & $[-0.102,\; -0.051]$ \\
\bottomrule
\end{tabular}
\end{table}

The paired difference of $\Delta = -0.076$ (neural policy achieves lower queue length) has a standard error of $0.013$, yielding a 95\% confidence interval of $[-0.102, -0.051]$ that excludes zero. However, because this neural policy is the best of 5 random initialization seeds selected on a held-out validation set, the observed advantage is subject to optimistic selection bias. Without per-seed ablation logs (which were not preserved), we cannot compute the unbiased ensemble mean or its corresponding significance level. Accordingly, we interpret this result as a \emph{directional signal} that behavior-cloning initialization from a strong teacher, combined with \reinforce{} fine-tuning, can match or marginally improve upon the closed-form teacher---but the true expected gain of a single randomly-initialized training run remains uncertain.
