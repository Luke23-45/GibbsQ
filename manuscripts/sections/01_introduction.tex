...
GibbsQ demonstrates that entropy regularization enables stability proofs for heterogeneous queue routing, and that the resulting analytical policies serve as effective teachers for learned policies. This connection unifies three findings:
...
Calibrated UAS, a three-parameter extension, achieves $\E[\sum_i \Qlen{i}] = 10.04$ under the standard benchmark, reducing steady-state queue length by $0.98$ units (8.84\%) relative to JSSQ.
...
Training from scratch diverges catastrophically ($5{,}451$), confirming that the analytical teacher---not the RL algorithm---is the dominant factor among the tested training design choices.
\end{enumerate}

The teacher-choice principle is this paper's most impactful finding for practitioners; the stability theorems provide its theoretical foundation.
