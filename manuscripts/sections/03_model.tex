...
This section defines the parallel-server queueing system, the routing problem, the four policy classes of the GibbsQ framework, and the stability criterion used in the theorem sections.
...
Upon each job arrival, a routing policy selects the destination server. Formally, a \emph{routing policy} is a measurable map
\begin{equation}
\label{eq:routing_map}
p \colon \Nats^\Nservers \to \Simplex,
\end{equation}
where $\Simplex = \bigl\{q \in [0,1]^\Nservers : \sum_{i=1}^\Nservers q_i = 1\bigr\}$ is the $(\Nservers-1)$-dimensional probability simplex. Under policy~$p$, an arriving job is routed to server~$i$ with probability $\rprob{i}(\Qstate)$.
...
The GibbsQ framework organizes routing policies into four layered classes: two analytical (provably stable) and two empirical.
...
\end{equation}
with benchmark parameters $(\calbeta, \calgamma, \caloffset) = (0.85, 0.5, 0.5)$. This family contains UAS as the special case $(\calbeta, \calgamma, \caloffset) = (1, 1, 1)$. Calibrated UAS is an \emph{empirical} closed-form extension and does not carry a formal stability guarantee; see \cref{sec:calibrated_uas}.
\end{definition}
...
The stability results in this paper use the continuous-time Foster--Lyapunov criterion from the Meyn--Tweedie framework~\cite{meyn2012markov,asmussen2003applied}.
...
Positive Harris recurrence ensures that steady-state performance metrics such as $\E[\sum_i \Qlen{i}]$ are well-defined and can be consistently estimated from simulation.
...
Both the raw softmax theorem (\cref{sec:softmax_theorem}) and the UAS theorem (\cref{sec:uas_theorem}) construct explicit Lyapunov functions and verify \cref{eq:foster_lyapunov} with explicit constants~$\varepsilon$ and~$R$.
...
$(\calbeta, \calgamma, \caloffset)$ & Calibrated UAS parameters \\
\bottomrule
\end{tabular}
\end{table}
