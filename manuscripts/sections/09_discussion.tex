...
The ablation study showcases a clear rank ordering under Protocol~B (where $\succ$ denotes strictly lower mean total queue length): Calibrated UAS teacher ($9.82$) $\succ$ UAS teacher ($11.14$) $\succ$ scratch ($5{,}451$; diverged).
...
Section~\ref{sec:calibrated_uas} identifies a theorem-backed certified subset within the calibrated family under the explicit sufficient condition $\varepsilon_{\calbeta,\calgamma,\caloffset}>0$.
...
An immediate empirical question is whether stronger RL optimizers preserve the same teacher-choice ordering observed here, or materially reduce the gap between UAS-teacher and calibrated-teacher initialization.
