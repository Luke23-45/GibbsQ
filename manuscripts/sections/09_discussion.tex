...
The $1.32$-unit gap between the Calibrated UAS teacher and the UAS teacher exceeds the impact of the three tested architectural modifications (log-normalization, zero-init, scratch initialization), which identifies teacher quality as a primary design factor in this empirical setting.
...
Calibrated UAS alters three key components of UAS:
\begin{itemize}
...
UAS's $0.328$), while the diminished offset increases responsiveness to current state---but a formal attribution of the $1.45$-unit improvement ($10.04$ vs.
...
\item \textbf{Calibrated UAS parameters are benchmark-specific:}
...
The teacher-choice principle parallels the pretraining-finetuning paradigm in foundation models: a strong analytical prior (Calibrated UAS) provides a high-quality initialization, and task-specific fine-tuning (REINFORCE) extracts marginal gains. This analogy suggests that the quality of the analytical prior may be more important than the fine-tuning algorithm, a hypothesis consistent with recent findings in large language model alignment.
