...
The Gini coefficient pattern is consistent with the interpretation that these adjustments improve steady-state performance by rebalancing load distribution. The tempered normalization and softened prior jointly reduce over-routing to fast servers, while the diminished offset increases responsiveness to current state. Calibrated UAS achieves $G = 0.174$ versus UAS's $G = 0.328$, a 47\% reduction in load imbalance, suggesting that the tempered parameters substantially balance routing mass across servers. This mirrors the theoretical insight extracted from the pairwise crossover law (derived earlier): explicitly reducing $\calbeta < 1$ shifts the routing boundary threshold toward the inherently faster servers, eliminating severe backlog accumulation on slower elements.
...
These findings confirm that the choice of teacher policy is the dominant factor in policy-gradient performance, but they also clarify that our reported neural "best" refers specifically to this calibrated-teacher variant, which requires separate behavior-cloning initialization. Additionally, interaction effects between ablation factors were not exhaustively tested; for instance, the combination of the Calibrated UAS teacher with architectural modifications (e.g., removing Log-Normalization) may yield further improvement.
...
A Bonferroni-adjusted 95\% CI (Table~\ref{tab:stats}) still excludes zero, but the unbiased ensemble mean remains unknown.
...
\item \textbf{Geometric ergodicity:}
While \cref{thm:uas} establishes positive Harris recurrence, the Foster--Lyapunov framework admits extensions to geometric ergodicity given standard geometric drift and minorization conditions~\cite[Chapter~15]{meyn2012markov}. Under the mild additional requirement of petite sets on compact state spaces, the bounded transition structure of the queue-length process suggests that exponential mixing rates may hold. Establishing this formally---particularly with explicit convergence rate constants---would strengthen the deployment argument and is an important open direction.
