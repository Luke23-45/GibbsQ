...
Temperature independence is an artifact of the conservative Jensen bound, which discards all $\temp$-dependence. This yields a uniform (though loose) guarantee across all $\temp > 0$. The drift constants $\varepsilon_{\mathrm{UAS}} = (\totcap - \arrate)/\totcap$ and $R_{\mathrm{UAS}} = \arrate\Nservers/\totcap + \Nservers/2$ depend only on $(\arrate, \srvrate_1, \dots, \srvrate_\Nservers)$. A tighter, $\temp$-dependent analysis would likely recover stronger bounds at higher temperatures.
