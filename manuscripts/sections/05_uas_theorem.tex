...
This section presents the second main analytical result: positive Harris recurrence of the CTMC under the Unified Archimedean Softmax (UAS) routing law. UAS is a heterogeneous extension of raw softmax that incorporates service rates through a prior-weighted variational objective and is analyzed with a weighted Lyapunov function. It is \emph{not} a corollary of \cref{thm:softmax}; it is a separate variational policy with a separate proof.
...
\end{equation}
which is equivalently written as $\rprob{i} = e^{-\temp \uaspot{i}} / \sum_j e^{-\temp \uaspot{j}}$. This is a smooth, capacity-aware, entropy-regularized routing policy for heterogeneous servers.
...
\mathcal{G}_{\mathrm{UAS}}(p) = \sum_{i=1}^{\Nservers} p_i\, \uasenergy{i}(\Qstate, \srvrate) + \frac{1}{\temp}\KL{p}{r} - \frac{1}{\temp}\log \totcap
\end{equation}
...
\end{equation}
Substituting into~\eqref{eq:uas_variational_obj} shows that $\mathcal{G}_{\mathrm{UAS}}(p)$ is algebraically identical to the UAS entropy-regularized objective. Strict convexity of $\mathcal{G}_{\mathrm{UAS}}$ in $p$ follows from the strict convexity of $\KL{p}{r}$ over the open simplex, guaranteeing a unique minimizer. The first-order optimality conditions on the simplex yield
\begin{equation}
\label{eq:uas_foc}
...
The UAS law combines three effects in a single score:
\begin{itemize}
\item \textbf{Queue pressure} through $\Qlen{i}$: longer queues receive less routing mass.
\item \textbf{Service heterogeneity} through $\srvrate_i$: the energy variable $\uasenergy{i} = (\Qlen{i}+1)/\srvrate_i$ normalizes queue length by service speed, and the prior $\uasprior{i} = \srvrate_i/\totcap$ favors faster servers.
\item \textbf{Entropy regularization} through the softmax form: routing remains smooth and interior rather than threshold-based.
\end{itemize}

At low load, the prior term dominates and UAS favors faster servers. Under congestion, the queue-dependent energy term shifts routing mass away from heavily loaded servers. The transition is smooth because routing probabilities remain exponential rather than discontinuous.
...
This smooth interpolation between exploitative and proportional routing at the idle state is a distinctive feature of UAS.
...
\end{equation}
Rearranging gives the exact crossover boundary at which routing mass transfers between the two servers:
\begin{equation}
...
For JSQ, the analogous crossover is $\Qlen{f} = \Qlen{s}$ (ignoring service rates entirely); for JSSQ, it is $\Qlen{f}/\srvrate_f = \Qlen{s}/\srvrate_s$.
...
This provides structural intuition for why Calibrated UAS with $\calbeta < 1$ may improve performance: tempering the normalization $\srvrate_i^{\calbeta}$ shifts the crossover boundary, effectively allowing more routing mass to remain with the faster server under moderate queue imbalance.
...
Combining the drift identity (\cref{lem:uas_drift}) with the arrival-term bound (\cref{lem:uas_arrival}),
\begin{align}
\gen \lyap_{\mathrm{UAS}}(\Qstate)
&\leq \arrate \frac{\abs{\Qstate}_1 + \Nservers}{\totcap} - \abs{\Qstate}_1 + \frac{\Nservers}{2} \nonumber \\
&= -\frac{\totcap - \arrate}{\totcap}\abs{\Qstate}_1 + \frac{\arrate \Nservers}{\totcap} + \frac{\Nservers}{2} \nonumber \\
&= -\varepsilon_{\mathrm{UAS}}\, \abs{\Qstate}_1 + R_{\mathrm{UAS}}.
...
\subsubsection{Independence from Raw Softmax}
...
\subsubsection{Role of the Prior}
...
\subsubsection{Temperature-Independent Stability Guarantee}
The drift constants $\varepsilon_{\mathrm{UAS}} = (\totcap - \arrate)/\totcap$ and $R_{\mathrm{UAS}} = \arrate\Nservers/\totcap + \Nservers/2$ are explicit functions of the system parameters $(\arrate, \srvrate_1, \dots, \srvrate_\Nservers)$ and are \emph{independent of the temperature $\temp$}.
This is a genuinely important structural advantage over the raw softmax bound, whose constant $R = (\arrate \log \Nservers)/\temp + C_1$ degrades as $\temp \to 0$ (high temperature).
Concretely, for UAS the compact set $C_{\mathrm{UAS}}$ and the drift rate $\varepsilon_{\mathrm{UAS}}$ do not change regardless of how much entropy regularization is applied.
The stability guarantee is therefore \emph{temperature-agnostic}: UAS is provably stable at the same rate for all $\temp > 0$, without requiring $\temp$ to be sufficiently large.

\subsubsection{Motivation for Empirical Extensions}
While UAS is provably stable, its specific parameterization $(\Qlen{i}+1)/\srvrate_i$ is not necessarily optimal for empirical performance. This observation motivates the Calibrated UAS extension in \cref{sec:calibrated_uas}, which explores a richer parameterization while sacrificing the theorem guarantee.
