...
\begin{proof}
Non-explosion and irreducibility follow by the same argument as in \cref{thm:softmax}: the total jump rate is bounded by $\arrate + \totcap < \infty$~\cite[Proposition~2.2.1]{meyn2012markov}, and every state communicates with every other state through a sequence of arrivals and departures because $\arrate > 0$ and $\srvrate_i > 0$ for all~$i$. The weighted Lyapunov function $\lyap_{\mathrm{UAS}}$ defined in~\eqref{eq:uas_lyapunov} is norm-like on $\Nats^\Nservers$.
...
\end{equation}
For the anchor benchmark evaluated in Section~\ref{sec:experiments} ($\Nservers=10, \rho=0.80$), this bound evaluates exactly to $65$. While this establishes formal stability analytically, the magnitude ($65$) is conservative compared to the actual empirical mean ($\approx 11.5$). The looseness stems from the Jensen relaxation $\sum \uasprior{i} e^{-\temp \uasenergy{i}} \ge \exp(-\temp \sum \uasprior{i} \uasenergy{i})$, which is tight only when all normalized loads $\uasenergy{i}$ are equal. Under the heterogeneous benchmark ($\srvrate_i$ ranging 0.5--2.3), the gap is large by design---this is the structural price of the temperature-independent guarantee.
\end{corollary}
