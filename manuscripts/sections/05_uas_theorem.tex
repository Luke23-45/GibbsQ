...
\end{equation}
The policy's name derives from the potential's $\frac{\Qlen{i}+1}{\srvrate_i}$ term, which acts structurally as a capacity-weighted proxy broadly comparable to an Archimedean generator function.\footnote{The analogy to Archimedean generators is structural and motivational, not algebraic---the UAS potential is not a copula generator and does not inherit copula properties.} Specifically, similar to how an Archimedean copula generates dependence structures via a monotonic generator function mapping to $[0,\infty)$, the UAS potential generates routing dependence via a monotonic mapping from state--capacity space to a strictly convex objective.
\end{definition}
...
\end{equation}
At large $\temp$ (low temperature, exploitative regime), $e^{-\temp/\srvrate_i} \to 0$ for slow servers while remaining close to $1$ for fast servers, so UAS heuristically approaches JSSQ-like behavior, concentrating routing mass on the fastest server.
