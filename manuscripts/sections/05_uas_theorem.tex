...
\begin{proof}
Non-explosion follows from Lemma~\ref{lem:non_explosion}. Irreducibility follows because every state communicates with every other state through a sequence of arrivals and departures since $\arrate > 0$ and $\srvrate_i > 0$ for all~$i$. The weighted Lyapunov function $\lyap_{\mathrm{UAS}}$ defined in~\eqref{eq:uas_lyapunov} is norm-like on $\Nats^\Nservers$.
...
\end{equation}
For the anchor benchmark evaluated in Section~\ref{sec:experiments} ($\Nservers=10, \rho=0.80$), this bound evaluates exactly to $65$. While this establishes formal stability analytically, the magnitude ($65$) is conservative compared to the actual empirical mean ($\approx 11.5$). The 5.6× looseness decomposes approximately as: (a) the Jensen relaxation contributes $\sim$2.5× (the quadratic Lyapunov bound on the arrival term vs.\ the actual nonlinear arrival contribution under heterogeneous service rates), and (b) the temperature-independent guarantee contributes the remaining $\sim$2.2× (the bound must hold for all $\temp > 0$, including near-zero temperatures where the softmax approaches uniform routing). This is the structural price of the temperature-independent guarantee.
\end{corollary}
...
Temperature independence is an artifact of the conservative Jensen bound, which discards all $\temp$-dependence. This yields a uniform (though loose) guarantee across all $\temp > 0$. The drift constants $\varepsilon_{\mathrm{UAS}} = (\totcap - \arrate)/\totcap$ and $R_{\mathrm{UAS}} = \arrate\Nservers/\totcap + \Nservers/2$ depend only on $(\arrate, \srvrate_1, \dots, \srvrate_\Nservers)$. A tighter, $\temp$-dependent analysis would likely recover stronger bounds at higher temperatures. Exploring Lagrangian dual formulations of the drift inequality may yield tighter certified regions.
...
The quadratic Lyapunov function $\lyap_{\mathrm{UAS}} = \frac{1}{2}\sum_i \Qlen{i}^2/\srvrate_i$ satisfies the stronger Foster--Lyapunov criterion for geometric ergodicity~\cite[Theorem~6.1]{meyn2012markov}: since $\gen \lyap_{\mathrm{UAS}}(\Qstate) \leq -\varepsilon_{\mathrm{UAS}} \abs{\Qstate}_1 + R_{\mathrm{UAS}}$ and $\lyap_{\mathrm{UAS}}(\Qstate) \geq c \abs{\Qstate}_1^2$ for some $c > 0$, the drift is quadratic-negative outside the compact set, implying geometric convergence to the stationary distribution.
