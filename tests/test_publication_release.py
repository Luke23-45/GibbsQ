...
def test_drift_verification_accepts_theorem_backed_policy_paths():
    from gibbsq.experiments.verification.drift_verification import _require_theorem_supported_policy
    ...
    with pytest.raises(ValueError, match="certifies only theorem-backed policy paths"):
        _require_theorem_supported_policy("reflected_uas")


def test_stability_sweep_remains_theorem_only():
    allowed = ["softmax", "uas"]

    assert "reflected_uas" not in allowed


def test_publication_neural_evaluation_runners_pin_reflected_uas_baseline():
...
def test_engine_parity_experiment_is_registered_and_policy_set_is_publication_safe():
    ...
    from gibbsq.qroute.core.config import EXPERIMENT_BLOCK_NAMES
    from gibbsq.experiments.verification.engine_parity import _policy_label, _policy_contract_alpha

    assert "engine_parity" in EXPERIMENTS
    assert "engine_parity" in EXPERIMENT_BLOCK_NAMES
    assert _policy_label("reflected_uas") == "Reflected UAS"
    assert _policy_contract_alpha("uas", 1.0) == pytest.approx(10.0)
    assert _policy_contract_alpha("reflected_uas", 1.0) == pytest.approx(20.0)
    assert _policy_contract_alpha("jsq", 1.0) == pytest.approx(1.0)


def test_hyperqual_summarizes_clean_policy_labels():
    from gibbsq.qroute.studies.hyperparameter_qualification import summarize_policy_results
    ...
    summary = summarize_policy_results(
        {
            ...
            "Reflected UAS": {"mean_q_total": 10.0},
        }
    )
    ...
    assert summary["reflected_uas_mean_q_total"] == pytest.approx(10.0)
...
def test_critical_regeneration_prefers_reflected_uas_metric(monkeypatch):
    ...
    (metrics_dir / "metrics.jsonl").write_text(
        '{"rho": 0.9, "neural_eq": 9.8, "reflected_uas_eq": 10.2, "gibbs_eq": 11.1}\n',
        encoding="utf-8",
    )
...
def test_critical_load_keeps_plot_baseline_series_numeric(monkeypatch):
    from gibbsq.experiments.evaluation.n_gibbsq_evals import critical_load
    ...
    monkeypatch.setattr("gibbsq.qroute.utils.model_io.validate_neural_model_shape", lambda *args, **kwargs: None)
    ...
    assert result["reflected_uas_eq"] == [7.0]
